...
const toggleMinigame = async (gameId: string, current: { enabled: boolean; max_reward: number }) => {
...
const updateMaxReward = async (gameId: string, current: { enabled: boolean; max_reward: number }, newValue: number) => {
  const newMinigames = { ...configQuery.data.value?.minigames };
...
const addMinigame = async () => {
  ...
  const newMinigames = { ...configQuery.data.value?.minigames };
  newMinigames[newGame.value.id.toLowerCase()] = { enabled: true, max_reward: newGame.value.max_reward };
  await updateConfig.mutateAsync({ minigames: newMinigames });
...
const getGameStats = (gameId: string) => {
  return minigamesQuery.data.value?.find(s => s.minigameId === gameId) || { totalRuns: 0, totalCoins: 0 };
};
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="configQuery.isLoading.value || minigamesQuery.isLoading.value" class="flex justify-center p-8">
      <ProgressSpinner />
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <div class="flex items-start justify-between mb-4">
          <div class="p-3 rounded-xl bg-indigo-500/10 text-indigo-400 group-hover:scale-110 transition-transform">
            <Gamepad2 :size="24" />
          </div>
          <ToggleSwitch 
        ...
        <h3 class="text-xl font-bold text-white capitalize mb-1">{{ gameId }}</h3>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="p-3 rounded-xl bg-slate-800/50 border border-slate-700/50">
            ...
            <div class="text-lg font-bold text-white">{{ getGameStats(gameId).totalRuns }}</div>
          </div>
          <div class="p-3 rounded-xl bg-slate-800/50 border border-slate-700/50">
            <div class="flex items-center gap-2 text-slate-400 text-xs mb-1">
              <Coins :size="14" />
            ...
            <div class="text-lg font-bold text-indigo-400">{{ getGameStats(gameId).totalCoins }}</div>
          </div>
        </div>

        <div class="mt-6 flex items-center justify-between text-xs">
          <span :class="settings.enabled ? 'text-green-400' : 'text-red-400'" class="flex items-center gap-1 font-medium">
            <span class="w-1.5 h-1.5 rounded-full" :class="settings.enabled ? 'bg-green-400' : 'bg-red-400'"></span>
