...
watch(() => configQuery.data.value, (newData) => {
  if (newData) {
    form.value.globalMultiplier = newData.globalMultiplier;
...
<template>
  <div class="space-y-6 max-w-3xl">
    ...
    <div v-else-if="configQuery.isError.value" class="text-red-400 p-4 bg-red-500/10 rounded-xl border border-red-500/20">
      ...
      <form class="space-y-6" @submit.prevent="saveConfig">
        ...
        <div class="space-y-4">
          <div class="flex flex-col gap-2">
            <label class="text-sm font-medium text-slate-300">Multiplicateur Global (XP/Coins)</label>
            <InputNumber
...
class="w-full sm:w-64 !bg-slate-950 !border-slate-800 !text-white" />
            <small class="text-slate-500">Applique un bonus à tous les joueurs (ex: 1.5 = +50%)</small>
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-sm font-medium text-slate-300">Niveau Maximum</label>
            <InputNumber v-model="form.maxLevel" input-id="maxLevel" class="w-full sm:w-64 !bg-slate-950 !border-slate-800 !text-white" />
          </div>

          <div class="flex items-center gap-2 mt-6">
            <Checkbox v-model="form.enablePurchases" input-id="enablePurchases" binary />
            <label for="enablePurchases" class="text-slate-300">Activer les achats en Robux (Boutique)</label>
          </div>

          <div class="flex items-center gap-2">
            <Checkbox v-model="form.enablePointContributions" input-id="enablePointContributions" binary />
            <label for="enablePointContributions" class="text-slate-300">Activer les contributions de points aux guerres</label>
          </div>
        </div>
