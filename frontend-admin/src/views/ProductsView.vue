...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-slate-400 text-sm">Monitoring et gestion du catalogue Roblox Marketplace.</p>
        </div>
      </div>

      <div class="flex flex-wrap items-center gap-4">
        <div class="flex items-center gap-4 px-4 py-2 rounded-xl bg-slate-900 border border-slate-800">
          ...
          <div class="flex flex-col">
            <span class="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Actifs</span>
            <span class="text-lg font-bold text-emerald-400">{{ stats.active }}</span>
          </div>
        </div>
        <Button label="Nouveau Produit" icon="pi pi-plus" @click="openCreateModal"
          class="!bg-indigo-600 !border-indigo-600 hover:!bg-indigo-500 transition-all shadow-lg shadow-indigo-600/20" />
      </div>
    </div>

    <!-- Filters & Search -->
    <div class="flex items-center justify-between gap-4 p-4 rounded-xl bg-slate-900 border border-slate-800">
      <div class="relative w-full max-w-md">
        <Search class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" :size="18" />
        <InputText v-model="filters['global'].value" placeholder="Rechercher un produit ou ID..."
          class="w-full !pl-10 !bg-slate-950 !border-slate-800" />
      </div>
      <div class="flex items-center gap-2">
        <Tag :value="`V${stats.total}.0`" severity="secondary" class="!bg-slate-800 !text-slate-400" />
      </div>
    </div>
    ...
    <div v-else class="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden shadow-2xl">
      <DataTable :value="productsQuery.data.value || []" v-model:filters="filters" paginator :rows="12" data-key="id"
        class="p-datatable-sm w-full" row-hover filter-display="menu"
        ...
        <template #empty>
          <div class="p-16 text-center text-slate-500 flex flex-col items-center gap-4">
            ...
            <p class="max-w-xs mx-auto">Aucun produit ne correspond à vos critères de recherche ou le catalogue est
              vide.</p>
            <Button label="Réinitialiser" link @click="filters['global'].value = null" v-if="filters['global'].value" />
          </div>
        </template>
        ...
        <Column field="name" header="Produit Monitoré" sortable>
          <template #body="{ data }">
            <div class="flex items-center gap-3">
              ...
              <div class="flex flex-col">
                <span class="font-bold text-white group-hover:text-indigo-400 transition-colors">{{ data.name }}</span>
                <span class="text-[10px] font-mono text-slate-500 uppercase tracking-tighter">REF: {{
                  data.robloxProductId }}</span>
              </div>
            </div>
          </template>
        </Column>
          ...
          <template #body="{ data }">
            <div class="group relative">
              ...
              <div
                ...
                <span class="text-[9px] text-indigo-400 font-bold uppercase tracking-widest">Voir détails</span>
              </div>
            </div>
          </template>
        </Column>
        ...
        <Column field="isActive" header="Status" sortable>
          <template #body="{ data }">
            <div class="flex items-center gap-2 px-2 py-1 rounded-full w-fit"
              :class="data.isActive ? 'bg-emerald-500/10' : 'bg-rose-500/10'">
              <div :class="data.isActive ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-rose-500'"
                class="w-1.5 h-1.5 rounded-full" />
              <span :class="data.isActive ? 'text-emerald-400' : 'text-rose-400'"
          ...
          <template #body="{ data }">
            <div class="flex items-center justify-end gap-1">
              <Button icon="pi pi-pencil" text rounded
                class="!text-slate-500 hover:!text-indigo-400 hover:!bg-indigo-500/5" @click="openEditModal(data)" />
            </div>
          </template>
        </Column>
      </DataTable>
    ...
    <ProductFormModal v-model:visible="isModalOpen" :product="selectedProduct" @save="handleSave"
