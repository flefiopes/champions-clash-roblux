...
const form = ref<Omit<CreateProductDto, 'value'> & { value: Record<string, any> }>({
  name: '',
  roblox_product_id: 0,
  price_robux: 0,
  type: 'gems',
  ...
  >
    <div class="space-y-8 pt-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          <div class="flex flex-col gap-4">
            <label
              class="text-xs font-bold text-slate-500 uppercase tracking-widest border-b border-slate-800 pb-2"
              >Configuration de Base</label
            >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="flex flex-col gap-1.5">
                ...
                <InputNumber
                  v-model="form.roblox_product_id"
                  :use-grouping="false"
          ...
          <div
            class="flex items-center justify-between p-4 rounded-xl bg-slate-950/50 border border-slate-800 shadow-inner"
          >
        ...
        <div class="space-y-6">
          <div class="p-5 rounded-xl bg-slate-950 border border-slate-800 space-y-5 shadow-lg">
            <h3
              class="text-xs font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-2 border-b border-slate-900 pb-3"
            >
            ...
            <div v-if="form.type === 'gems'" class="flex flex-col gap-2">
              ...
              <InputNumber v-model="form.value.gems" class="w-full" input-class="w-full" />
            </div>
            ...
            <div v-if="form.type === 'boost'" class="space-y-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="flex flex-col gap-2">
                  ...
                  <InputNumber
                    v-model="form.value.duration_seconds"
                    class="w-full"
                    input-class="w-full"
                  />
                </div>
            ...
            <div v-if="form.type === 'faction_reset'">
              <p
                class="text-xs text-slate-400 leading-relaxed italic bg-slate-900/50 p-3 rounded-lg border border-slate-800"
              >
                Action système automatisée : Réinitialise l'affiliation et les points de faction de
                l'utilisateur.
              </p>
            </div>
          </div>
          ...
          <div class="p-4 rounded-xl bg-slate-900/80 border border-slate-800/50 shadow-inner">
            <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 block"
              >Real-time Payload Analytics</label
            >
            <pre
              class="text-[10px] text-emerald-400/90 font-mono bg-black/60 p-4 rounded-lg overflow-x-auto max-h-[140px] scrollbar-thin scrollbar-thumb-slate-800 border border-slate-800/50"
            ><code>{{ JSON.stringify(form.value, null, 2) }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 pt-6 border-t border-slate-800/50">
        <Button
          label="Annuler"
          class="p-button-text !text-slate-400 hover:!text-white transition-colors"
